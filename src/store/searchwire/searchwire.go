...
func EncodeTextBoolRequest(indexName string, must, should, mustNot, filter []TextClause, minShould uint16, boost float32, limit, offset uint32) []byte {
	...
	cursor += 2
	binary.LittleEndian.PutUint16(out[cursor:], uint16(len(filter)))
	cursor += 2
	...
	encodeClauses(out, &cursor, filter)
	return out
}
...
func DecodeTextBoolRequest(raw []byte) (TextBoolRequest, error) {
	...
	cursor = next
	filter, next, err := decodeClauses(raw, cursor, filterCount)
	if err != nil {
		return TextBoolRequest{}, err
	}
	if next != len(raw) {
...
func encodedClausesLen(clauses []TextClause) int {
	...
	for _, clause := range clauses {
		...
		total += encodedClausesLen(clause.Filter)
	}
	return total
}
...
func encodeClauses(out []byte, cursor *int, clauses []TextClause) {
	for _, clause := range clauses {
		...
		*cursor += 4
		binary.LittleEndian.PutUint32(out[*cursor:], uint32(len(clause.AltText)))
		*cursor += 4
		...
		*cursor += 2
		binary.LittleEndian.PutUint16(out[*cursor:], uint16(len(clause.Filter)))
		*cursor += 2
		...
		*cursor += len(clause.Text)
		copy(out[*cursor:], clause.AltText)
		...
		*cursor += len(clause.Relation)
		...
		encodeClauses(out, cursor, clause.Filter)
	}
}
...
func decodeClauses(raw []byte, cursor int, count int) ([]TextClause, int, error) {
	...
	for i := 0; i < count; i++ {
		...
		cursor += 4
		altTextLen := int(binary.LittleEndian.Uint32(raw[cursor : cursor+4]))
		cursor += 4
		...
		cursor += 2
		filterCount := int(binary.LittleEndian.Uint16(raw[cursor : cursor+2]))
		cursor += 2
		...
		text := string(raw[cursor : cursor+textLen])
		cursor += textLen
		altText := string(raw[cursor : cursor+altTextLen])
		cursor += altTextLen
		analyzer := string(raw[cursor : cursor+analyzerLen])
		cursor += analyzerLen
		parser := string(raw[cursor : cursor+parserLen])
